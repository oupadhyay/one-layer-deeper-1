...
PATH = Path(__file__).resolve().parents[1] / "submissions/sgu_register_medium_m5_v1/submission.py"
loader = importlib.util.spec_from_file_location("sgu_register_medium_m5_v1", PATH)
...
class SGURegisterMediumTests(unittest.TestCase):
    ...
    def test_five_digit_lsd_parser_tail_and_rejections(self):
        ...
        self.assertEqual(info["widths"].item(), 5)
        self.assertEqual((out[0, 10:15, 7:17] > m.NEG).sum().item(), 50)
        bad = [
