...
PATH = ROOT / "submissions/factorized_pair_reducer_e5_remote_v3_d32k1/submission.py"
V2_PATH = ROOT / "submissions/factorized_pair_reducer_e5_remote_v2_k1/submission.py"
module_spec = importlib.util.spec_from_file_location("factorized_pair_reducer_e5_remote_v3_d32k1", PATH)
...
class FactorizedPairReducerTests(unittest.TestCase):
    def test_state_seed_optimizer_and_source(self):
        ...
        self.assertEqual(count_model_state_elements(a), 17002)
        ...
        expected = v2.replace("D, PLACES, MICROPHASES, MAX_STEPS = 64, 4, 1, 64", "D, PLACES, MICROPHASES, MAX_STEPS = 32, 4, 1, 64")
        expected = expected.replace("STATE_ELEMENTS = 64_714", "STATE_ELEMENTS = 17_002")
        self.assertEqual(source, expected)

        self.assertEqual(candidate.D, 32)
        self.assertEqual(a.digit_embedding.weight.shape, (10, 32))
        self.assertEqual(a.pair_encoder.mlp[0].weight.shape, (32, 64))
        self.assertEqual(a.pair_encoder.mlp[2].weight.shape, (32, 32))
        self.assertEqual(a.reducer.qkv.weight.shape, (96, 32))
