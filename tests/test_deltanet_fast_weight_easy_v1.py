...
PATH = Path(__file__).resolve().parents[1] / "submissions/deltanet_fast_weight_easy_v1/submission.py"
loader = importlib.util.spec_from_file_location("deltanet_easy", PATH)
...
def sequential(q, k, v, beta, state):
    ...
    for i in range(q.shape[-2]):
        ...
        outputs.append(torch.einsum("...k,...kv->...v", q[..., i, :], state) / (24 ** .5))
...
class DeltaNetTests(unittest.TestCase):
    def test_chunk_oracle_outputs_state_and_gradients(self):
        torch.manual_seed(4)
        values = [torch.randn(2, 3, 12, 24, dtype=torch.float64, requires_grad=True) for _ in range(3)]
        beta = torch.randn(2, 3, 12, dtype=torch.float64, requires_grad=True)
        state = torch.randn(2, 3, 24, 24, dtype=torch.float64, requires_grad=True)
    ...
    def test_state_optimizer_tying_and_reset(self):
        ...
        self.assertEqual(count_model_state_elements(model), 96106)
    ...
    def test_parser_alignment_and_macrosteps(self):
        ...
        for name, p in model.named_parameters(): self.assertIsNotNone(p.grad, name)
