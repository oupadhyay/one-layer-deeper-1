...
import competition_loop12_factorized_pair_reducer_d96k3 as candidate
...
class FactorizedPairReducerTests(unittest.TestCase):
    def test_state_seed_optimizer_and_source(self):
        ...
        self.assertEqual(count_model_state_elements(a), 143722)
        self.assertLess(count_model_state_elements(a), 250000)
        self.assertEqual((candidate.D, candidate.MICROPHASES), (96, 3))
        self.assertEqual(tuple(a.scratch_tokens.shape), (8, 96))
        self.assertIs(a.reducer, a.reducer)
    ...
    def test_parsing_context_pairs_counts_and_feedback(self):
        ...
        self.assertEqual(debug["pair_token_count"],57); self.assertEqual(debug["reducer_calls"],9); self.assertEqual(debug["scratch_resets"],3)
