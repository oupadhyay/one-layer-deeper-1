...
PROVEN = ROOT / "submissions/compressed_axial_e5_remote_v7_lr3e4/submission.py"
PATH = ROOT / "submissions/compressed_axial_e5_remote_v8_t1only/submission.py"
...
candidate = load(PATH, "compressed_axial_e5_remote_v8_t1only")
proven = load(PROVEN, "compressed_axial_e5_remote_v7_lr3e4_reference")


class CompressedAxialE5RemoteV8T1OnlyTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_exact_promotion_source_diff(self):
        ...
        old = "logits.new_tensor(0.01)"
        new = "logits.new_tensor(0.0)"
        ...
        changed = [(left, right) for left, right in zip(proven_source.splitlines(), candidate_source.splitlines())
                   if left != right]
        self.assertEqual(changed, [("            scale = torch.where(steps.eq(1), logits.new_tensor(1.0), logits.new_tensor(0.01))",
    ...
    def test_forward_logits_unchanged_and_gradient_gate(self):
        torch.manual_seed(1234)
        reference = proven.build_model(self.spec)
        model = candidate.build_model(self.spec)
        ...
        for step in (1, 2, 3):
            ...
            reference_logits[:, -3:, proven.DIGIT:proven.DIGIT + 10].sum().backward()
