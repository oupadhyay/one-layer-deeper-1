...
PATH = Path(__file__).resolve().parents[1] / "submissions/sgu_register_easy_v1/submission.py"
loader = importlib.util.spec_from_file_location("sgu_register_easy_v1", PATH)
m = importlib.util.module_from_spec(loader); loader.loader.exec_module(m)


class SGURegisterTests(unittest.TestCase):
    def setUp(self):
