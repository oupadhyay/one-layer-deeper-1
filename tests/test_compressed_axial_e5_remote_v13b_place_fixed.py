...
PATH = Path(__file__).resolve().parents[1] / "submissions/compressed_axial_e5_remote_v13b_place_fixed/submission.py"
spec = importlib.util.spec_from_file_location("v13b_place_fixed", PATH)
...
def test_contract_validation_state_and_no_routing():
    ...
    assert counts == [candidate.STATE_ELEMENTS] * 3 == [49_722] * 3
...
def test_places_distinguishable_shared_and_boundary_canonical():
    ...
    assert x[0, 0, 3, 80].item() == candidate.BOUNDARY_PLACE == 64
