...
PATH = ROOT / "submissions/compressed_axial_e5_remote_v15_c64_batch128/submission.py"
...
candidate = load(PATH, "compressed_axial_e5_remote_v15_c64_batch128")
proven = load(PROVEN, "compressed_axial_e5_remote_v10_c64_lr1e4_reference")


class CompressedAxialE5RemoteV15C64Batch128Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_exact_promotion_source_diff(self):
        ...
        old = "batch_size=512, eval_batch_size=512"
        new = "batch_size=128, eval_batch_size=128"
        self.assertEqual(proven_source.count(old), 1)
        candidate_source = PATH.read_text(encoding="utf-8")
        self.assertEqual(candidate_source, proven_source.replace(old, new))
        ...
        self.assertEqual(changed, [
            ("SUBMISSION = Submission(build_model, build_optimizer, batch_size=512, eval_batch_size=512, max_steps=None)",
             "SUBMISSION = Submission(build_model, build_optimizer, batch_size=128, eval_batch_size=128, max_steps=None)")])
    ...
    def test_batch128_t1_t2_t3_forward(self):
        model = candidate.build_model(self.spec)
        ids = torch.cat([self.inputs(step) for step in (1, 2, 3)] * 42 +
                        [self.inputs(1), self.inputs(2)])
        self.assertEqual(ids.shape[0], 128)
        logits, info = model(ids)
        self.assertEqual(logits.shape, (128, ids.shape[1], 17))
        self.assertEqual(info["parsed_steps"].bincount(minlength=4).tolist(), [0, 43, 43, 42])
    ...
    def test_exact_optimizer_and_official_submission_contract(self):
        ...
        self.assertEqual((candidate.SUBMISSION.batch_size, candidate.SUBMISSION.eval_batch_size,
                          candidate.SUBMISSION.max_steps), (128, 128, None))
