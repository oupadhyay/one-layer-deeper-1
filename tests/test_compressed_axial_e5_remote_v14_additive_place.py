...
PATH = Path(__file__).resolve().parents[1] / "submissions/compressed_axial_e5_remote_v14_additive_place/submission.py"
spec = importlib.util.spec_from_file_location("v14_additive_place", PATH)
...
def test_contract_validation_state_and_no_routing():
    ...
    counts = []
    for max_seq_len in (20, 32, 64):
        model = candidate.build_model(ModelSpec(17, max_seq_len, 1_000_000))
        counts.append(count_model_state_elements(model))
        assert model.place_embedding.num_embeddings == candidate.PLACE_TABLE_SIZE == 65
    assert counts == [candidate.STATE_ELEMENTS] * 3 == [47_674] * 3
    assert model.initializer[0].in_features == 70
...
def test_places_distinguishable_shared_and_boundary_canonical():
    ...
    with torch.no_grad():
        model.place_embedding.weight.copy_(torch.arange(65)[:, None].expand(-1, 16))
    ...
    x = captured["x"]
    # ei/ej remain their original 16-wide fields and receive place vectors additively.
    assert not torch.equal(x[0, 1, 1, :16], x[0, 2, 1, :16])
    torch.testing.assert_close(x[0, 1, 1, :16], x[0, 1, 1, 16:32])
    # Boundary ej has no digit contribution: it is exactly shared table row 64.
    torch.testing.assert_close(x[0, 0, 3, 16:32], x[1, 0, 2, 16:32])
    assert x[0, 0, 3, 16].item() == candidate.BOUNDARY_PLACE == 64
    # ni/nj and flags retain the v10 layout and boundary masking.
    assert torch.count_nonzero(x[0, 0, 3, 48:64]) == 0
    assert x.shape[-1] == 70
