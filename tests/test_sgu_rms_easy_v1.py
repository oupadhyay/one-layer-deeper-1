...
PATH = Path(__file__).resolve().parents[1] / "submissions/sgu_rms_easy_v1/submission.py"
loader = importlib.util.spec_from_file_location("sgu_rms_easy_v1", PATH)
m = importlib.util.module_from_spec(loader); loader.loader.exec_module(m)


class SGURMSTests(unittest.TestCase):
    def setUp(self):
        torch.manual_seed(112)
        self.model = m.build_model(ModelSpec(17, 24, 250_000))

    def test_exact_state_api_init_and_optimizer(self):
        self.assertEqual(count_model_state_elements(self.model), 79_490)
        ...
        self.assertIsInstance(block.norm, m.RMSNorm)
        self.assertIsInstance(block.gate_norm, m.RMSNorm)
        self.assertIsInstance(self.model.transition.readout_norm, m.RMSNorm)
        self.assertEqual((block.norm.eps, block.gate_norm.eps, self.model.transition.readout_norm.eps), (1e-6, 1e-6, 1e-6))
        self.assertFalse(any(isinstance(module, torch.nn.LayerNorm) for module in self.model.modules()))
