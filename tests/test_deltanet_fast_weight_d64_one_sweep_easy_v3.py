...
PATH = Path(__file__).resolve().parents[1] / "submissions/deltanet_fast_weight_d64_one_sweep_easy_v3/submission.py"
SOURCE_PATH = Path(__file__).resolve().parents[1] / "submissions/deltanet_fast_weight_d64_easy_v2/submission.py"
loader = importlib.util.spec_from_file_location("deltanet_d64_one_sweep_easy", PATH)
...
class DeltaNetOneSweepTests(unittest.TestCase):
    ...
    def test_source_is_exact_one_sweep_control_and_d64_only(self):
        source = PATH.read_text(encoding="utf-8")
        expected = SOURCE_PATH.read_text(encoding="utf-8").replace(
            "D64 two-sweep chunkwise", "D64 one-sweep chunkwise"
        ).replace("for _ in range(2):", "for _ in range(1):")
