...
BASE = ROOT / "submissions/neural_gpu_e5_remote_v1_t1/submission.py"
PATH = ROOT / "submissions/neural_gpu_m5_remote_v2_compositional/submission.py"
loader = importlib.util.spec_from_file_location("neural_gpu_m5", PATH)
...
class NeuralGpuM5CompositionalTests(unittest.TestCase):
    ...
    def test_contract_state_optimizer_and_source(self):
        ...
        self.assertEqual(candidate.STATE_ELEMENTS, 126_682)
    ...
    def test_train_eval_parity_counts_and_resets(self):
        torch.manual_seed(12)
        model = candidate.build_model(spec())
        ids = torch.cat((row(n=(1, 2, 3, 4), t=2), row(n=(8, 7), t=4)))
        calls = []
        ...
        self.assertEqual((aux["macrosteps"], aux["microticks_per_macrostep"]), (4, 12))
        self.assertEqual((aux["phase_a_calls"], aux["phase_b_calls"]), (24, 24))
        self.assertEqual(calls, ["A", "B"] * 24)
        self.assertEqual((aux["scratch_resets"], aux["n_reinitializations"]), (4, 4))
        self.assertEqual(eval_aux["macrosteps"], aux["macrosteps"])

    def test_t2_t4_t8_plain_endpoint_gradients(self):
    ...
    def test_t64_deterministic_pure_and_max_t(self):
        ...
        self.assertEqual((aux["parsed_steps"].item(), aux["macrosteps"],
                          aux["scratch_resets"], aux["n_reinitializations"]), (64, 64, 64, 64))
