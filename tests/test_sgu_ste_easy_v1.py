...
PATH = Path(__file__).resolve().parents[1] / "submissions/sgu_ste_easy_v1/submission.py"
loader = importlib.util.spec_from_file_location("sgu_ste_easy_v1", PATH)
m = importlib.util.module_from_spec(loader); loader.loader.exec_module(m)


class SGUSTETests(unittest.TestCase):
    def setUp(self):
    ...
    def test_register_order_soft_embedding_absence_and_sgu_oracle(self):
        ...
        oracle = z + block.project(u * block.spatial(block.gate_norm(v).transpose(1,2)).transpose(1,2))
    ...
    def test_source_validation_and_exclusions(self):
        ...
        lower = source.lower()
        for forbidden in ("scaled_dot_product", "covariance", "conv1d", "scan", "router", "remainder", "cross_entropy", "cache"):
            self.assertNotIn(forbidden, lower)

    def test_feedback_ste_forward_and_gradient_semantics(self):
        soft = torch.softmax(torch.randn(2, 4, 10, requires_grad=True), -1)
        hard = torch.nn.functional.one_hot(soft.argmax(-1), 10).to(soft.dtype)
        feedback = hard + soft - soft.detach()
        torch.testing.assert_close(feedback, hard)
        gradient, = torch.autograd.grad(feedback.sum(), soft)
        torch.testing.assert_close(gradient, torch.ones_like(soft))
