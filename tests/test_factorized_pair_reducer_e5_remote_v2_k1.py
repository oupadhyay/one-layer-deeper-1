...
PATH = ROOT / "submissions/factorized_pair_reducer_e5_remote_v2_k1/submission.py"
V1_PATH = ROOT / "submissions/factorized_pair_reducer_e5_remote_v1/submission.py"
module_spec = importlib.util.spec_from_file_location("factorized_pair_reducer_e5_remote_v2_k1", PATH)
...
class FactorizedPairReducerTests(unittest.TestCase):
    def test_state_seed_optimizer_and_source(self):
        ...
        self.assertEqual(count_model_state_elements(a), 64714)
        ...
        expected = v1.replace("MICROPHASES, MAX_STEPS = 64, 4, 2, 64", "MICROPHASES, MAX_STEPS = 64, 4, 1, 64")
        expected = expected.replace("STATE_ELEMENTS = 64_778", "STATE_ELEMENTS = 64_714")
