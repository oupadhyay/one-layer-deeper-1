...
PROVEN = ROOT / "submissions/cuda_bisect_axial_full_forward_v1/submission.py"
PATH = ROOT / "submissions/compressed_axial_e5_remote_v5/submission.py"


def load():
    spec = importlib.util.spec_from_file_location("compressed_axial_e5_remote_v5", PATH)
...
class CompressedAxialE5RemoteV5Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_exact_promotion_source_diff(self):
        ...
        expected = proven.replace(
            "MAX_STEPS, CHANNELS, STATE_ELEMENTS = 3, 32, 13162",
            "MAX_STEPS, CHANNELS, STATE_ELEMENTS = 64, 32, 13162",
        ).replace(
            "eval_batch_size=128, max_steps=1)",
            "eval_batch_size=128, max_steps=None)",
        )
    ...
    def test_t1_t2_t3_gradients_finite(self):
        for step in (1, 2, 3):
