...
PATH = Path(__file__).resolve().parents[1] / "submissions/deltanet_fast_weight_d64_three_sweep_easy_v4/submission.py"
SOURCE_PATH = Path(__file__).resolve().parents[1] / "submissions/deltanet_fast_weight_d64_easy_v2/submission.py"
loader = importlib.util.spec_from_file_location("deltanet_d64_three_sweep_easy", PATH)
...
class DeltaNetThreeSweepTests(unittest.TestCase):
    ...
    def test_state_optimizer_tying_and_reset(self):
        ...
        with torch.no_grad():
            calls = 0
            def count_block(_module, _args, _result):
                nonlocal calls
                calls += 1
            handle = model.transition.block.register_forward_hook(count_block)
    ...
    def test_source_is_exact_three_sweep_control_and_d64_only(self):
        source = PATH.read_text(encoding="utf-8")
        expected = SOURCE_PATH.read_text(encoding="utf-8").replace(
            "D64 two-sweep chunkwise", "D64 three-sweep chunkwise"
        ).replace("for _ in range(2):", "for _ in range(3):")
