...
PROVEN = ROOT / "submissions/compressed_axial_e5_remote_v6_batch512/submission.py"
PATH = ROOT / "submissions/compressed_axial_e5_remote_v7_lr3e4/submission.py"


def load():
    spec = importlib.util.spec_from_file_location("compressed_axial_e5_remote_v7_lr3e4", PATH)
...
class CompressedAxialE5RemoteV7Lr3e4Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_exact_promotion_source_diff(self):
        ...
        old = "model.parameters(), lr=1e-3, betas=(0.9, 0.95)"
        new = "model.parameters(), lr=3e-4, betas=(0.9, 0.95)"
        self.assertEqual(proven.count(old), 1)
        candidate_source = PATH.read_text(encoding="utf-8")
        self.assertEqual(candidate_source, proven.replace(old, new))
        changed = [(left, right) for left, right in zip(proven.splitlines(), candidate_source.splitlines())
                   if left != right]
        self.assertEqual(changed, [("    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, betas=(0.9, 0.95),",
    ...
    def test_batch512_t1_t2_t3_forward_backward(self):
        model = candidate.build_model(self.spec)
