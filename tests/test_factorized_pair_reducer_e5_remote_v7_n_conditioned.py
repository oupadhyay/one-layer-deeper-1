...
PATH = ROOT / "submissions/factorized_pair_reducer_e5_remote_v7_n_conditioned/submission.py"
module_spec = importlib.util.spec_from_file_location("factorized_pair_reducer_v7", PATH)
...
class NConditionedPairReducerTests(unittest.TestCase):
    def test_masked_immutable_summary_is_repeated_for_every_pair(self):
        ...
        captured = []
        handle = model.n_conditioner[0].register_forward_pre_hook(
        ...
        torch.testing.assert_close(captured[0][..., candidate.D:],
                                   expected_summary[:, None].expand(-1, 16, -1))
    ...
    def test_state_gradients_gate_optimizer_purity_max_t_and_validation(self):
        ...
        model = candidate.build_model(spec())
        self.assertEqual(count_model_state_elements(model), 77_130)
        self.assertEqual(sum(p.numel() for p in model.n_conditioner.parameters()), 12_416)
        model.train()
