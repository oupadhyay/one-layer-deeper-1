"""Factorized ordered digit-pair context with a tied learned reducer."""
...
D, PLACES, MICROPHASES, MAX_STEPS = 64, 4, 2, 64
STATE_ELEMENTS = 64_778
...
class Model(nn.Module):
    ...
    def debug_execution(self, input_ids, attention_mask=None):
        ...
        return {"parsed_steps": steps, "widths": widths, "active_macrosteps": int(steps.sum()),
                "pair_token_count": int((widths * widths * steps).sum()),
                "reducer_calls": int(MICROPHASES * steps.max()), "scratch_resets": int(steps.max())}
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        for macro in range(int(steps.max().item())):
            ...
            all_feedback.append(probabilities)
        ...
        aux = {**self.debug_execution(input_ids, attention_mask), "digit_probabilities": probabilities,
               "feedback_states": all_feedback, "plain_endpoint_ce": True}
        return logits, aux
