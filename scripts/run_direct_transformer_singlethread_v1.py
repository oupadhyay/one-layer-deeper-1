"""Deterministic single-thread four-block direct Transformer protocol v1."""
...
body=body.replace("import competition_loop2_latent as candidate","import competition_loop9_direct_transformer as candidate")
body=body.replace('ROOT / "competition_loop2_latent.py"','ROOT / "competition_loop9_direct_transformer.py"')
body=body.replace("latent_exposure_singlethread_v1","direct_transformer_singlethread_v1")
exec(compile(body,str(parent_path),"exec"),globals())

FROZEN_BISCAN_COST=687.6041304974351/1600
PROBE1=ARTIFACTS/"direct_transformer_singlethread_v1_probe1.json"
PROBE2=ARTIFACTS/"direct_transformer_singlethread_v1_probe2.json"

def capacity_preflight(spec):
    x=torch.tensor([[2,10,9,10,3,8,12,4,8],[2,10,9,10,3,15,16,4,9],[2,10,9,10,3,11,9,4,10]])
    labels=torch.tensor([[9,9,12],[8,10,14],[9,16,7]]); positions=torch.tensor([[6,7,8]]*3); bi=torch.arange(3)[:,None]
    ...
    result={"rows":3,"true_t":[1,2,3],"optimizer":"AdamW(lr=3e-3)","maximum_updates":100,"updates_run":update,"train_correct":train_correct,"same_weight_eval_correct":int(evaluated.argmax(-1).eq(labels).all(1).sum())}
...
def train_update(model,bundle,batch,manifest):
    if _horizons is not None:
        ...
        _horizons.update(map(int,values.tolist())); _counts["direct_model_calls"]+=1
        _counts["transformer_block_calls"]+=4
...
def probe(output:Path):
    ...
    payload={"protocol":"direct_transformer_singlethread_v1","settings":settings,"preflight":preflight,"losses":losses,"ordered_batch_digests":batches,
             ...
             "batch_horizon_histogram":{f"T{k}":v for k,v in sorted(_horizons.items())},"call_counts":dict(_counts),"loss":"ordinary full CE; no horizon gradient gate"}
    artifact={"comparison_payload":payload,"timing":{"candidate_training_seconds_per_update":seconds,"biscan_frozen_seconds_per_update":FROZEN_BISCAN_COST,"ratio":seconds/FROZEN_BISCAN_COST}}
...
def _authorize_full_run():
    ...
    if ratio>3: raise RuntimeError("resource ratio rejects implementation")
...
def full_run():
    ...
    final=result["final_evaluation"]; t1=result["milestones"]["1600"]["evaluation"]["train_by_t"]["T1"]; correct,ce=t1["correct_examples"],t1["loss"]
    verdict="PASS" if correct>=60 or ce<=1.10 else ("REJECT" if correct<=25 and ce>=1.45 else "INCONCLUSIVE")
    test=final["test_by_t"]["T1"]; seen=final["seen_N_T1"]; ood=final["depth_ood_n_t_1"]
    transfer=test["correct_examples"]>=10 and test["loss"]<4.1123304367 and seen["correct_examples"]>=4 and ood["correct_examples"]>3
    competition=result["gates"]; competition_pass=competition.get("competition_pass",False)
    result["gates"]={"competition":competition,"one_step_learnability":{"verdict":verdict,"correct":correct,"cross_entropy":ce},"transfer":{"pass":transfer,"test_t1":test,"seen_n_t1":seen,"ood_n_t1":ood},"promotion_pass":competition_pass and transfer}
    result.setdefault("provenance",{})["dynamic_dependencies"]={str(parent_path.relative_to(ROOT)):sha256(parent_path),"competition_loop9_direct_transformer.py":sha256(SOURCE),"benchmark/s2_c1.py":sha256(ROOT/"benchmark/s2_c1.py")}
