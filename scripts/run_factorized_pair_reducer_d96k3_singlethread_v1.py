"""Deterministic bounded preflight for the D96/K3 factorized pair reducer."""
...
body=body.replace("competition_loop6_biscan_t1grad","competition_loop12_factorized_pair_reducer_d96k3")
body=body.replace("biscan_e5_diversity_singlethread_v1","factorized_pair_reducer_d96k3_singlethread_v1")
exec(compile(body,str(parent_path),"exec"),globals())

FROZEN_BISCAN_COST=.034
PROBE1=ARTIFACTS/"factorized_pair_reducer_d96k3_singlethread_v1_probe1.json"
PROBE2=ARTIFACTS/"factorized_pair_reducer_d96k3_singlethread_v1_probe2.json"
...
def probe(path:Path):
    ...
    payload["protocol"]="factorized_pair_reducer_d96k3_singlethread_v1"
...
TRAINABILITY_JSON=ARTIFACTS/"factorized_pair_reducer_d96k3_singlethread_v1_trainability400.json"
TRAINABILITY_PT=ARTIFACTS/"factorized_pair_reducer_d96k3_singlethread_v1_trainability400.pt"
...
def authorize_trainability_after_review():
    ...
    if ratio>3.0: raise RuntimeError("resource ratio rejects trainability screen")
    return {"decision":"trainability-only review authorization","maximum_ratio":ratio,
            "projected_400_update_seconds":400*max(first["timing"]["candidate_training_seconds_per_update"],
                                                     second["timing"]["candidate_training_seconds_per_update"]),
...
def trainability():
    resource_review=authorize_trainability_after_review()
    ...
    state["protocol"]="factorized_pair_reducer_d96k3_singlethread_v1_trainability400"
    ...
    result={"status":"complete","protocol":"factorized_pair_reducer_d96k3_singlethread_v1_trainability400",
      ...
      "correction_history":["The initial probes entered the predeclared 2.5-to-3.0 review band. A trainability-only review authorized 400 updates after the projected runtime fit the bounded CPU budget. The 1,600-update path remained blocked."],
      "resource_review":resource_review,"determinism":settings,"preflight":preflight,
