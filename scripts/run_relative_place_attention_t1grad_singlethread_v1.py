"""Deterministic single-thread relative-place-attention T1-gradient protocol v1."""
...
# Mechanically reuse the reviewed complete-resume harness while pinning it below.
...
body=body.replace("import competition_loop2_latent as candidate","import competition_loop7_relative_place_attention_t1grad as candidate")
body=body.replace('ROOT / "competition_loop2_latent.py"','ROOT / "competition_loop7_relative_place_attention_t1grad.py"')
body=body.replace("latent_exposure_singlethread_v1","relative_place_attention_t1grad_singlethread_v1")
exec(compile(body,str(parent_path),"exec"),globals())

FROZEN_BISCAN_COST=687.6041304974351/1600
PROBE1=ARTIFACTS/"relative_place_attention_t1grad_singlethread_v1_probe1.json"
PROBE2=ARTIFACTS/"relative_place_attention_t1grad_singlethread_v1_probe2.json"
...
def train_update(model,bundle,batch,manifest):
    if _horizons is not None:
        ...
        _counts["active_row_macrosteps"]+=int(values.sum()); _counts["vectorized_block_calls"]+=maximum
        _counts["attention_block_calls"]+=maximum; _counts["readout_calls"]+=maximum
...
def probe(output:Path):
    ...
    losses,batches=[],[]; epoch=consumed=examples=0; batches_per_epoch=len(loaders["train"])
    ...
    payload={"protocol":"relative_place_attention_t1grad_singlethread_v1","settings":settings,
    ...
    payload["batch_horizon_histogram"]={f"T{k}":v for k,v in sorted(_horizons.items())}
    payload["macrostep_call_counts"]=dict(_counts)
    payload["backward_gate"]={"T1":1.0,"T_not_1":0.01,"forward_values_ungated":True}
    artifact={"comparison_payload":payload,"timing":{"candidate_training_seconds_per_update":seconds_per_update,
              "biscan_frozen_seconds_per_update":FROZEN_BISCAN_COST,
              "ratio":seconds_per_update/FROZEN_BISCAN_COST}}
...
def full_run():
    ...
    t1=result["milestones"]["1600"]["evaluation"]["train_by_t"]["T1"]; correct,ce=t1["correct_examples"],t1["loss"]
    ...
    transfer=test["correct_examples"]>=10 and test["loss"]<4.1123304367 and seen["correct_examples"]>=4 and seen["loss"]<4.8310351372 and ood["correct_examples"]>3 and ood["loss"]<4.4054730842
    competition=result["gates"].get("competition",result["gates"])
    competition_pass=competition.get("competition_pass",False)
    ...
    result.setdefault("provenance",{})["dynamic_dependencies"]={str(parent_path.relative_to(ROOT)):sha256(parent_path),"scripts/run_latent_exposure_singlethread_v1.py":sha256(ROOT/"scripts/run_latent_exposure_singlethread_v1.py"),"competition_loop7_relative_place_attention_t1grad.py":sha256(SOURCE)}
