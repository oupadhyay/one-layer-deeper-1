"""Standalone Medium M3 axial SGU with LayerNorm."""
...
STATE_ELEMENTS,NEG=80_033,-10_000.0
class Config:
...
class SGURegister(nn.Module):
 def __init__(self):
  super().__init__(); self.digit_embedding=nn.Embedding(10,D_MODEL); self.role_embedding=nn.Embedding(2,D_MODEL); self.place_embedding=nn.Embedding(WIDTH,D_MODEL); self.presence_embedding=nn.Embedding(2,D_MODEL); self.block=SpatialGatingBlock(); self.readout_norm=nn.LayerNorm(D_MODEL); self.readout=nn.Linear(D_MODEL,10)
...
class Model(nn.Module):
 ...
 def forward(self,ids,attention_mask=None):
  ...
  for i in range(runs):
   proposed,reg=self.transition(n,np,prob,present); active=(steps>i)[:,None,None]; end=torch.where(active,proposed,end)
   soft=proposed.softmax(-1); hard=F.one_hot(soft.argmax(-1),10).to(soft.dtype); feedback=hard-soft.detach()+soft if self.training else hard
   confident=soft.max(-1,keepdim=True).values.ge(.5); zero=F.one_hot(torch.zeros_like(n),10).to(soft.dtype); feedback=torch.where(confident,feedback,zero)
   prob=torch.where(active,feedback,prob); present=torch.where(active[:,:,0],np,present)
...
def build_optimizer(model,spec):
 ...
 o=torch.optim.AdamW([{"params":d,"weight_decay":0.0},{"params":n,"weight_decay":0.}],lr=6e-4,betas=(.9,.95),eps=1e-8,capturable=spec.device_type=="cuda"); return OptimizerBundle(o,torch.optim.lr_scheduler.LambdaLR(o,lambda u:min((u+1)/32,1.)))
