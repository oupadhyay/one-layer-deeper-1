"""Dynamic-width learned memory indexed by the squaring-preserving reflection orbit."""
...
class Model(nn.Module):
    ...
    def decode(self, n_int, orbit, width):
        key_a = torch.remainder(
            (n_int * 6364136223846793005).bitwise_xor(orbit * 1442695040888963407),
            TABLE,
        )
        key_b = torch.remainder(
            (n_int * 3202034522624059733).bitwise_xor(orbit * 3935559000370003845),
            TABLE,
        )
    ...
    def forward(self, ids, attention_mask=None):
        ...
        if length > self.config.max_seq_len or bool(
            ((nw < 1) | (xw < 1) | (nw > 18) | (xw > 18) | (steps > 64)).any()
        ):
        ...
        for tick in range(runs):
            ...
            x_int = (x_digits.round().long() * powers).sum(1)
            orbit = (2 * x_int - n_int).abs()
            proposed, key = self.decode(n_int, orbit, width)
