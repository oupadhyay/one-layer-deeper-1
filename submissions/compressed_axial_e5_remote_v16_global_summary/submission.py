"""Compressed axial baseline with zero-initialized global pair summaries."""
...
MAX_STEPS, CHANNELS, STATE_ELEMENTS = 64, 64, 54954
...
class Model(nn.Module):
    def __init__(self, spec):
        ...
        self.summary_norm = nn.RMSNorm(2 * CHANNELS)
        self.summary_out = nn.Linear(2 * CHANNELS, CHANNELS, bias=False)
    ...
    @staticmethod
    def _summaries(workspace, widths):
        ...
        pair_mask = (rows < widths[:, None, None]) & (cols < widths[:, None, None])
        weighted = workspace * pair_mask[..., None]
        row_mean = weighted.sum(2) / pair_mask.sum(2).clamp_min(1)[..., None]
        global_mean = weighted.sum((1, 2)) / pair_mask.sum((1, 2)).clamp_min(1)[..., None]
        return row_mean, global_mean[:, None].expand(-1, workspace.shape[1], -1)
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        for macro in range(int(steps.max().item())):
            ...
            row_mean, global_mean = self._summaries(workspace, widths)
            boundary = boundary + self.summary_out(self.summary_norm(torch.cat((row_mean, global_mean), -1)))
