"""Baseline Transformer plus four axial-cell calls and a diagnostic readout."""
...
MAX_STEPS, D_MODEL, NUM_HEADS, CHANNELS = 64, 128, 4, 32
...
class Model(nn.Module):
    ...
    def _workspace(self, probabilities: Tensor, n_digits: Tensor, widths: Tensor) -> tuple[Tensor, Tensor]:
    ...
    @staticmethod
    def _boundary(workspace: Tensor, widths: Tensor) -> Tensor:
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None) -> tuple[Tensor, None]:
        ...
        workspace = self.cell(workspace, "H") * valid[..., None]
        workspace = self.cell(workspace, "V") * valid[..., None]
        readout_logits = self.readout(self._boundary(workspace, widths))
        ...
        readout_zero = readout_logits.sum().to(logits.dtype) * 0.0
        return logits + parser_zero + readout_zero, None
...
def build_optimizer(model: nn.Module, spec: OptimizerSpec) -> OptimizerBundle:
    return OptimizerBundle(torch.optim.AdamW(model.parameters(), lr=1e-3, betas=(0.9, 0.95),
...
SUBMISSION = Submission(build_model=build_model, build_optimizer=build_optimizer,
                        batch_size=64, eval_batch_size=128, max_steps=1)
