"""Dynamic-width query-pooled relation bank with a shared recurrent decoder."""
...
def position_features(z):return torch.stack((z,z.square(),torch.sin(math.pi*z),torch.cos(math.pi*z),torch.sin(2*math.pi*z),torch.cos(2*math.pi*z),torch.sin(4*math.pi*z),torch.cos(4*math.pi*z)),-1)
...
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  n,np=field(1);x,xp=field(2);ne,initial=s.transition.encode_n(n,np);prob=F.one_hot(x,10).to(s.transition.xdigit.weight.dtype);end=prob.clamp_min(1e-8).log();present=xp;runs=int(t.max().item());state=None
