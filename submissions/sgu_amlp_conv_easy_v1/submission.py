"""Attention-augmented spatial-gating categorical register for the Easy tier."""
...
STATE_ELEMENTS, NEG = 89_346, -10_000.0
...
class SpatialGatingBlock(nn.Module):
    """gMLP SGU with one shared learned operator over the eight token positions."""
    def __init__(self) -> None:
        ...
        self.qkv = nn.Linear(D_MODEL, 48, bias=False)
        self.attention_out = nn.Linear(16, HIDDEN, bias=False)
    ...
    def forward(self, register: Tensor) -> Tensor:
        ...
        q, k, value = self.qkv(register).chunk(3, dim=-1)
        weights = (q @ k.transpose(1, 2) / 4.0).softmax(dim=-1)
        gate = gate + self.attention_out(weights @ value)
...
class SGURegister(nn.Module):
    ...
    def forward(self, n: Tensor, n_present: Tensor, x: Tensor,
        ...
        register = register + self.local(register.transpose(1, 2)).transpose(1, 2)
