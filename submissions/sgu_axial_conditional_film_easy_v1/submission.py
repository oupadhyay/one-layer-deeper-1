"""Axial spatial gating with learned global N-conditioned modulation."""
...
class SpatialGatingBlock(nn.Module):
    def __init__(self) -> None:
        ...
        self.expand = nn.Linear(D_MODEL, 2 * HIDDEN)
        ...
        self.context_film = nn.Linear(D_MODEL, 2 * HIDDEN)
        self.project = nn.Linear(HIDDEN, D_MODEL)
    ...
    def forward(self, register: Tensor, n_present: Tensor) -> Tensor:
        u, v = F.gelu(self.expand(self.norm(register))).chunk(2, dim=-1)
        shaped = register.reshape(register.shape[0], WIDTH, 2, D_MODEL)
        weight = n_present[..., None].to(register.dtype)
        summary = (shaped[:, :, 0] * weight).sum(1) / weight.sum(1).clamp_min(1.)
        scale, shift = self.context_film(self.context_norm(summary)).chunk(2, -1)
        u = u * (1 + torch.tanh(scale[:, None])) + shift[:, None]
        ...
        gate = (place_gate + role_gate + self.spatial_bias[None, :, :, None]).reshape_as(u)
...
class SGURegister(nn.Module):
    ...
    def forward(self, n: Tensor, n_present: Tensor, x: Tensor,
        ...
        register = self.block(self.make_register(n, n_present, x, x_present), n_present)
...
SUBMISSION = Submission(build_model, build_optimizer, batch_size=256,
                        eval_batch_size=256, max_steps=None)
