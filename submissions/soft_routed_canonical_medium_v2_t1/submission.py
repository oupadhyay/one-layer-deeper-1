"""Digit-only recurrent transition with learned localized soft routing."""
...
class Transition(nn.Module):
    def __init__(self):
        ...
        self.place = nn.Embedding(W, D)
        self.encoder = nn.Sequential(nn.Linear(2 * W * D, H), nn.GELU(), nn.Linear(H, H), nn.GELU())
        self.router = nn.Linear(H, EXPERTS)
        self.base = nn.Linear(H, W * 10)
        self.experts = nn.Parameter(torch.empty(EXPERTS, W * 10))
        self.identity_scale = nn.Parameter(torch.empty(()))
        nn.init.normal_(self.experts, std=.02)
        nn.init.normal_(self.identity_scale, std=.02)

    def forward(self, n, np, probability):
        dtype = self.digit.weight.dtype
        n_state = self.digit(n) + self.presence(np.long()) + self.place.weight[None]
        x_state = probability.to(dtype) @ self.digit.weight + self.place.weight[None]
        hidden = self.encoder(torch.cat((n_state, x_state), -1).flatten(1))
        routing = (self.router(hidden).float() / .2).softmax(-1).to(dtype)
        local = routing @ self.experts
        identity = probability.to(dtype).flatten(1) * self.identity_scale
        return (self.base(hidden) + local + identity).view(-1, W, 10), routing
...
class Model(nn.Module):
    ...
    def forward(self, ids, attention_mask=None):
        ...
        for tick in range(runs):
            proposed, routing = self.transition(n, np, probability)
