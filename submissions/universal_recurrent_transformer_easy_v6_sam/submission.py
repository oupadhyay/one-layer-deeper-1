"""D128 universal recurrent Transformer with evaluator-owned two-pass SAM."""
...
class SAMAdamW(torch.optim.AdamW):
    """AdamW whose evaluator-controlled first pass supplies a SAM perturbation."""
    ...
    def perturb(self) -> None:
        ...
        for parameter in parameters:
            e_w = parameter.grad.detach() * scale.to(parameter.device)
            parameter.add_(e_w)
...
class SAMBetweenBackwardPasses:
    """Current-API callback; the evaluator invokes this between its passes."""

    def __init__(self, optimizer: SAMAdamW) -> None:
        self.optimizer = optimizer

    @torch.no_grad()
    def __call__(self, context) -> None:
        if context.pass_index == 0:
            self.optimizer.perturb()
...
def build_optimizer(model: nn.Module, spec: OptimizerSpec) -> OptimizerBundle:
    ...
    current_api = "backward_passes_per_step" in OptimizerBundle.__dataclass_fields__
    optimizer_class = SAMAdamW if current_api else torch.optim.AdamW
    ...
    if current_api:
        return OptimizerBundle(
            ...
            between_backward_passes=SAMBetweenBackwardPasses(optimizer),
            should_reuse_batch=None,
        )
