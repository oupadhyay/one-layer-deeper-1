"""One-block axial spatial-gating categorical register for the Easy tier."""
...
STATE_ELEMENTS, NEG = 79_910, -10_000.0
...
class SpatialGatingBlock(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.rank_left = nn.Parameter(torch.empty(2 * WIDTH))
        self.rank_right = nn.Parameter(torch.empty(2 * WIDTH))
        ...
        nn.init.uniform_(self.rank_left, -1.25e-4, 1.25e-4)
        nn.init.uniform_(self.rank_right, -1.25e-4, 1.25e-4)
    ...
    def forward(self, register: Tensor) -> Tensor:
        ...
        flat = v.reshape(v.shape[0], 2 * WIDTH, HIDDEN)
        correction = self.rank_left[None, :, None] * torch.einsum("i,bih->bh", self.rank_right, flat)[:, None, :]
        gate = (place_gate + role_gate + self.spatial_bias[None, :, :, None]).reshape_as(u) + correction
