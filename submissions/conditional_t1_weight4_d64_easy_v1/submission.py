"""Dynamic conditional T1-weight screen: weight4_d64."""
...
def build_optimizer(m,spec):
 ...
 o=torch.optim.AdamW([{'params':d,'weight_decay':.01},{'params':n,'weight_decay':0.}],lr=9e-4,betas=(.9,.95),eps=1e-8,capturable=spec.device_type=='cuda');return OptimizerBundle(o,torch.optim.lr_scheduler.LambdaLR(o,lambda u:min((u+1)/8,1.)))
def token_training_loss(batch:TokenLossBatch):
 ce=F.cross_entropy(batch.logits.transpose(1,2),batch.labels,ignore_index=-100,reduction='none');v=batch.valid_mask;seq=(ce*(1-torch.exp(-ce)).pow(.5)*v).sum(1)/v.sum(1).clamp_min(1);w=torch.where(batch.auxiliary['steps'].eq(1),seq.new_tensor(4.),seq.new_tensor(1.));return (seq*w).sum()/w.sum()
