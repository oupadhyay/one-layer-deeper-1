"""Local probe: exact even-remainder coordinates with a learned recurrence."""
...
class Transition(nn.Module):
    def __init__(self):
        super().__init__()
        self.log_sharpness = nn.Parameter(torch.randn(()) + 12)
    ...
    def forward(self, n, x):
        ...
        n_phase = 2 * math.pi * n_remainder / periods[None]
        resonance = torch.exp(-self.log_sharpness.float().exp() * (1 - n_phase.cos()))
        x_remainder = torch.remainder(x[:, None], periods[None])
        x_even_remainder = torch.minimum(x_remainder, periods[None] - x_remainder)
        x_even = (2 * math.pi * x_even_remainder / periods[None]).cos()
        features = torch.cat((resonance, resonance * x_even), -1)
        return self.network(features.to(self.network[0].weight.dtype)).view(-1, W, 10), resonance
