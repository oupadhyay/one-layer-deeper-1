"""Persistent globally mixed polynomial recurrence over a whole-number latent state."""
from __future__ import annotations
import torch
import torch.nn.functional as F
from torch import nn
from benchmark import OptimizerBundle,Submission,TokenLossBatch,assert_model_state
W,D,R,INNER=4,32,128,2
class C:
...
class Transition(nn.Module):
 def __init__(s):
  super().__init__();s.digit=nn.Embedding(10,D);s.presence=nn.Embedding(2,D);s.place=nn.Embedding(W,D);s.initial=nn.Linear(W*D,R);s.context=nn.Linear(W*D,3*R);s.mix=nn.Linear(R,R,bias=False);s.out=nn.Linear(R,W*10)
...
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  for step in range(runs):
   proposed=state
   for _ in range(INNER):proposed=s.transition.step(proposed,coefficients)
   active=(t>step)[:,None];state=torch.where(active,proposed,state);end=torch.where(active[:,:,None],s.transition.out(state).view(B,W,10),end)
