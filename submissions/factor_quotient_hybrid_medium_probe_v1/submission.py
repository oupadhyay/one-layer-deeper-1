...
class Transition(nn.Module):
    def __init__(self):
        ...
        self.shared = nn.Sequential(
            nn.Linear(12, 256), nn.GELU(), nn.Linear(256, 256), nn.GELU(),
        )
        self.shared_digits = nn.Linear(256, W * 10)
        self.shared_value = nn.Linear(256, 1)
    ...
    def forward(self, n, x):
        ...
        factor_values = factor_indices.float() + 2
        residue_values = residues.float()
        p, q = factor_values.unbind(-1)
        rp, rq = residue_values.unbind(-1)
        features = torch.stack((p / 64, q / 64, rp / p, rq / q,
                                (rp / p).square(), (rq / q).square(),
                                rp * rq / (p * q), (p - q) / 64,
                                (rp + rq) / 64, (rp - rq) / 64,
                                x / n, n / 4096), -1)
        hidden = self.shared(features.to(self.shared[0].weight.dtype))
        key = (((factor_indices[:, 0] * PERIODS + factor_indices[:, 1]) * RESIDUES
                + residues[:, 0]) * RESIDUES + residues[:, 1])
        key = torch.remainder(key, TABLE_SIZE)
        table_logits = self.table(key).view(-1, W, 10)
        shared_logits = self.shared_digits(hidden).view(-1, W, 10)
        return table_logits, shared_logits, factor, self.shared_value(hidden).sigmoid().squeeze(-1)
...
class Model(nn.Module):
    ...
    def forward(self, ids, attention_mask=None):
        ...
        for tick in range(runs):
            ...
            table_proposed, shared_proposed, resonance, value_proposal = self.transition(n_int, x_int)
            ...
            predicted_value = torch.where(active[:, 0, 0], value_proposal, predicted_value if predicted_value is not None else value_proposal)
            shared_endpoint = torch.where(active, shared_proposed, shared_endpoint if shared_endpoint is not None else shared_proposed)
        ...
        return torch.where(occupied[..., None], digits, logits), {
            ...
            "predicted_value": predicted_value, "modulus": n_int,
...
def token_training_loss(batch: TokenLossBatch):
    ...
    shared_ce = F.cross_entropy(shared[selected].transpose(1, 2),
                                batch.labels[selected], ignore_index=-100, reduction="none")
    shared_classification = (shared_ce * valid).sum() / valid.sum()
    ...
    target_value = target_value.float() / batch.auxiliary["modulus"][selected].float()
    regression = F.mse_loss(batch.auxiliary["predicted_value"][selected].float(), target_value)
    return classification + shared_classification + 10 * regression
