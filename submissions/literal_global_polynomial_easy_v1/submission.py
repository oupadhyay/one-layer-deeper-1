"""Literal N-conditioned polynomial recurrence over one whole-number latent state."""
...
class Transition(nn.Module):
 def __init__(s):
  super().__init__();s.digit=nn.Embedding(10,D);s.presence=nn.Embedding(2,D);s.place=nn.Embedding(W,D);s.initial=nn.Linear(W*D,R);s.context=nn.Linear(W*D,3*R);s.norm=RMS(R);s.out=nn.Linear(R,W*10)
 def encode(s,digit,present):return (s.digit(digit)+s.presence(present.long())+s.place.weight[None]).flatten(1)
 def step(s,state,coefficients):
  a,b,c=coefficients;return s.norm(torch.tanh(a)*state.square()+torch.tanh(b)*state+c)
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  n,np=field(1);x,xp=field(2);coefficients=s.transition.context(s.transition.encode(n,np)).chunk(3,-1);state=torch.tanh(s.transition.initial(s.transition.encode(x,xp)));end=state.new_zeros(B,W,10);runs=int(t.max().item())
  for step in range(runs):
   proposed=s.transition.step(state,coefficients);active=(t>step)[:,None];state=torch.where(active,proposed,state);end=torch.where(active[:,:,None],s.transition.out(state).view(B,W,10),end)
