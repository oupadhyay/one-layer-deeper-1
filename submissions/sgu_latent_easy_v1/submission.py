"""One-block spatial-gating register with persistent latent X tokens."""
...
STATE_ELEMENTS, NEG = 79_938, -10_000.0
...
class SGULatent(nn.Module):
    ...
    def tokens(self, value: Tensor, present: Tensor, role: int) -> Tensor:
        dtype = self.digit_embedding.weight.dtype
        content = self.digit_embedding(value) * present[..., None].to(dtype)
        return (content + self.role_embedding.weight[role] + self.place_embedding.weight[None]
                + self.presence_embedding(present.long()))

    def initial_x(self, x: Tensor, present: Tensor) -> Tensor:
        return self.tokens(x, present, 1)

    def forward(self, n: Tensor, n_present: Tensor, x_latent: Tensor) -> tuple[Tensor, Tensor]:
        register = torch.cat((self.tokens(n, n_present, 0), x_latent), dim=1)
        register = self.block(register)
        latent = register[:, WIDTH:]
        return self.readout(self.readout_norm(latent)), latent
...
class Model(nn.Module):
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None):
        ...
        n, n_present = field(1)
        x, x_present = field(2)
        latent = self.transition.initial_x(x, x_present)
        endpoint = F.one_hot(x, 10).to(latent.dtype).clamp_min(1e-8).log()
        runs = 3 if self.training else int(steps.max().item())
        for macrostep in range(runs):
            proposed, transformed = self.transition(n, n_present, latent)
            active = (steps > macrostep)[:, None, None]
            endpoint = torch.where(active, proposed, endpoint)
            latent = torch.where(active, transformed, latent)
