"""Easy E5 Neural-GPU: a recurrent, local four-row convolutional grid."""
...
class Model(nn.Module):
    def __init__(self, spec):
        ...
        self.phase_a = CGRUCell()
        self.phase_b = CGRUCell()
    ...
    def transition(self, x_state, n_digits):
        ...
        for _ in range(p + 2):
            grid = self.phase_a(grid)
            grid = self.phase_b(grid)
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        auxiliary = {"parsed_steps": steps, "widths": widths, "active_places": p,
                     ...
                     "phase_a_calls": calls // 2, "phase_b_calls": calls // 2,
                     "cell_call_order": ("A", "B") * (p + 2), "grid_shape": (input_ids.shape[0], 4, p, CHANNELS),
