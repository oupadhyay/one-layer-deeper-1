"""BiScan entropy preference with hard-forward straight-through feedback."""
...
class Model(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        for iteration in range(executed):
            ...
            hard_feedback = F.one_hot(soft_feedback.argmax(-1), 10).to(soft_feedback.dtype)
            feedback = (hard_feedback + soft_feedback - soft_feedback.detach()
                        if self.training else hard_feedback)
