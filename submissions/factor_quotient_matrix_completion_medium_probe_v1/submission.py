...
W, PERIODS, RESIDUES, TABLE_SIZE, RANK = 4, 63, 33, 131071, 32
...
class Transition(nn.Module):
    def __init__(self):
        ...
        self.left = nn.Embedding(PERIODS * RESIDUES, RANK * W * 10)
        self.right = nn.Embedding(PERIODS * RESIDUES, RANK * W * 10)
        nn.init.normal_(self.table.weight, std=.02)
        nn.init.normal_(self.left.weight, std=.02)
        nn.init.normal_(self.right.weight, std=.02)
    ...
    def forward(self, n, x):
        ...
        left = self.left(left_key).view(-1, RANK, W, 10)
        right = self.right(right_key).view(-1, RANK, W, 10)
        shared = (left * right).sum(1) / math.sqrt(RANK)
        return self.table(key).view(-1, W, 10), shared, factor
...
class Model(nn.Module):
    ...
    def forward(self, ids, attention_mask=None):
        ...
        for tick in range(runs):
            ...
            table_proposed, shared_proposed, resonance = self.transition(n_int, x_int)
...
def token_training_loss(batch: TokenLossBatch):
    ...
    shared_ce = F.cross_entropy(shared[selected].transpose(1, 2), batch.labels[selected],
                                ignore_index=-100, reduction="none")
    return classification + (shared_ce * valid).sum() / valid.sum()
