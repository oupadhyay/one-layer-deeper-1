"""Standalone Medium M3 dense SGU with the C21 normalization recipe."""
...
STATE_ELEMENTS,NEG=79_864,-10_000.0
class Config:
...
class SpatialGatingBlock(nn.Module):
 def __init__(self):
  super().__init__(); self.norm=RMSNorm(D_MODEL); self.expand=nn.Linear(D_MODEL,2*HIDDEN); self.gate_norm=nn.LayerNorm(HIDDEN); self.spatial=nn.Linear(10,10); self.project=nn.Linear(HIDDEN,D_MODEL)
  nn.init.uniform_(self.spatial.weight,-1.25e-4,1.25e-4); nn.init.ones_(self.spatial.bias)
 def forward(self,x):
  u,v=F.gelu(self.expand(self.norm(x))).chunk(2,-1); g=self.spatial(self.gate_norm(v).transpose(1,2)).transpose(1,2); return x+self.project(u*g)
