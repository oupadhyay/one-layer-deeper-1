...
class PromptEncoder(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor) -> Tensor:
        x = x + self.attention(self.attention_norm(x), key_mask=mask)
...
class RecurrentBlock(nn.Module):
    ...
    def forward(self, state: Tensor, prompt: Tensor, prompt_mask: Tensor) -> Tensor:
        state = state + RESIDUAL_SCALE * self.self_attention(self.self_norm(state))
        state = state + RESIDUAL_SCALE * self.cross_attention(
            self.cross_norm(state), prompt, prompt_mask
        )
        return state + RESIDUAL_SCALE * self.ffn(self.ffn_norm(state))
...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.quantizer_norm = RMSNorm()
        self.selector = nn.Linear(D, K, bias=False)
        self.codebook = nn.Parameter(torch.empty(K, D))
    ...
    def quantize(self, h: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        """Return the quantized vectors, ST counts, and hard counts."""
        logits = self.selector(self.quantizer_norm(h))
        probs = F.softmax(logits, dim=-1)
        hard = F.one_hot(logits.argmax(dim=-1), K).to(dtype=probs.dtype)
        assignment = hard + probs - probs.detach() if self.training else hard
        counts = assignment.float().sum(dim=(0, 1))
        hard_counts = hard.detach().float().sum(dim=(0, 1))
        return assignment @ self.codebook, counts, hard_counts
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None):
        ...
        identity = torch.cat((outputs, scratch), dim=1)
        state = identity
        counts = torch.zeros(K, device=state.device, dtype=torch.float32)
        hard_counts = torch.zeros_like(counts)
        for _ in range(REFINEMENTS):
            h = self.recurrent_block(state, encoded, mask)
            quantized, depth_counts, depth_hard_counts = self.quantize(h)
            counts = counts + depth_counts
            hard_counts = hard_counts + depth_hard_counts
            state = quantized + identity
        state_length = length + SCRATCH
        mean_usage = counts / (REFINEMENTS * batch * state_length)
        usage_loss = 0.01 * K * ((mean_usage - 1.0 / K) ** 2).sum()
        logits = self.vocabulary_projection(self.final_norm(state[:, :length]))
        return logits, {
            "usage_loss": usage_loss,
            "occupancy": (hard_counts > 0).sum().detach(),
...
def training_loss(logits: Tensor, labels: Tensor, auxiliary: object) -> Tensor:
    return F.cross_entropy(logits, labels) + auxiliary["usage_loss"]
