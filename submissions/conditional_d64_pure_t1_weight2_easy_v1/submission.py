"""Dynamic D64 conditional transducer with pure T1 token weighting."""
...
def token_training_loss(batch:TokenLossBatch):
 ce=F.cross_entropy(batch.logits.transpose(1,2),batch.labels,ignore_index=-100,reduction='none');v=batch.valid_mask;w=torch.where(batch.auxiliary['steps'].eq(1),ce.new_tensor(2.),ce.new_tensor(1.))[:,None];return (ce*(1-torch.exp(-ce)).pow(.5)*v*w).sum()/(v*w).sum()
