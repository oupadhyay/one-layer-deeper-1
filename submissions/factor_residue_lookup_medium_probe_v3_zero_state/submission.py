...
class Model(nn.Module):
    ...
    def forward(self, ids, attention_mask=None):
        ...
        return torch.where(occupied[..., None], digits, logits), {
            ...
            "macrosteps": runs, "resonance": resonance, "output_positions": occupied,
        }
...
def token_training_loss(batch: TokenLossBatch):
    ...
    occupied = batch.auxiliary["output_positions"][selected]
    labels = batch.labels[selected]
    labels = torch.where(occupied & ~batch.valid_mask[selected], 7, labels)
    if not bool(occupied.any()):
        return batch.logits.sum() * 0
    ce = F.cross_entropy(batch.logits[selected].transpose(1, 2), labels,
                         ignore_index=-100, reduction="none")
    return (ce * occupied).sum() / occupied.sum()
