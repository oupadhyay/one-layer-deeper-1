"""Local probe: categorical recurrence over N-conditioned periodic resonances."""
...
class Transition(nn.Module):
    def __init__(self):
        super().__init__()
        self.log_sharpness = nn.Parameter(torch.randn(()) + 7)
        self.network = nn.Sequential(
            nn.Linear(2 * PERIODS, H), nn.GELU(),
            nn.Linear(H, H), nn.GELU(), nn.Linear(H, W * 10),
        )

    def forward(self, n, x):
        dtype = self.network[0].weight.dtype
        periods = torch.arange(2, 65, device=x.device, dtype=dtype)
        n = n.to(dtype)
        x = x.to(dtype)
        n_phase = 2 * math.pi * n[:, None] / periods[None]
        resonance = torch.exp(-self.log_sharpness.exp() * (1 - n_phase.cos()))
        x_even = (2 * math.pi * x[:, None] / periods[None]).cos()
        features = torch.cat((resonance, resonance * x_even), -1)
        return self.network(features).view(-1, W, 10), resonance
...
class Model(nn.Module):
    ...
    def forward(self, ids, attention_mask=None):
        ...
        for tick in range(runs):
            x_digits = (probability * digit_values).sum(-1)
            x_int = (x_digits * powers.to(x_digits.dtype)).sum(1)
