"""Dynamic-width tied bidirectional learned transducer for Hard H1."""
...
class Block(nn.Module):
 def __init__(s):
  super().__init__();s.norm=RMS(D);s.expand=nn.Linear(D,2*H);s.gnorm=nn.LayerNorm(H);s.scan_in=nn.Linear(2*H,32,bias=False);s.scan=nn.GRU(32,32,batch_first=True,bidirectional=True);s.scan_out=nn.Linear(64,2*H,bias=False);s.role=nn.Parameter(torch.empty(2,2));s.bias=nn.Parameter(torch.ones(2));s.project=nn.Linear(H,D);nn.init.uniform_(s.role,-1.25e-4,1.25e-4)
 def forward(s,x,w):
  u,v=F.gelu(s.expand(s.norm(x))).chunk(2,-1);v=s.gnorm(v).reshape(x.shape[0],w,2,H);z=s.scan_in(v.reshape(x.shape[0],w,2*H));z,_=s.scan(z);p=s.scan_out(z).reshape(x.shape[0],w,2,H);g=(p+torch.einsum('or,bprh->bpoh',s.role,v)+s.bias[None,None,:,None]).reshape_as(u);return x+s.project(u*g)
class Transition(nn.Module):
 def __init__(s):super().__init__();s.digit=nn.Embedding(10,D);s.role=nn.Embedding(2,D);s.presence=nn.Embedding(2,D);s.block=Block();s.outnorm=RMS(D);s.out=nn.Linear(D,10)
 def forward(s,n,np,x,xp):
  w=n.shape[1];dt=s.digit.weight.dtype;a=s.digit(n)*np[...,None].to(dt);b=(x.to(dt)@s.digit.weight)*xp[...,None].to(dt);r=torch.stack((a+s.role.weight[0]+s.presence(np.long()),b+s.role.weight[1]+s.presence(xp.long())),2).reshape(n.shape[0],2*w,D);r=s.block(r,w);return s.out(s.outnorm(r[:,1::2])),r
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  if L>s.config.max_seq_len or bool(((nw<1)|(xw<1)|(t>64)).any()):raise ValueError('invalid input contract')
  ...
  n,np=field(1);x,xp=field(2);prob=F.one_hot(x,10).to(s.transition.digit.weight.dtype);end=prob.clamp_min(1e-8).log();present=xp;runs=int(t.max().item());reg=None
  for i in range(runs):
