"""Medium M3 axial LayerNorm SGU with under-relaxed feedback."""
...
class Block(nn.Module):
 def __init__(s):
  super().__init__();s.norm=nn.LayerNorm(D);s.expand=nn.Linear(D,2*H);s.gnorm=nn.LayerNorm(H);s.place=nn.Parameter(torch.empty(W,W));s.role=nn.Parameter(torch.empty(W,2,2));s.bias=nn.Parameter(torch.ones(W,2));s.project=nn.Linear(H,D);nn.init.uniform_(s.place,-1.25e-4,1.25e-4);nn.init.uniform_(s.role,-1.25e-4,1.25e-4)
 def forward(s,x):
  u,v=F.gelu(s.expand(s.norm(x))).chunk(2,-1);v=s.gnorm(v).reshape(x.shape[0],W,2,H);g=(torch.einsum('op,bprh->borh',s.place,v)+torch.einsum('por,bprh->bpoh',s.role,v)+s.bias[None,:,:,None]).reshape_as(u);return x+s.project(u*g)
class Transition(nn.Module):
 def __init__(s):super().__init__();s.digit=nn.Embedding(10,D);s.role=nn.Embedding(2,D);s.place=nn.Embedding(W,D);s.presence=nn.Embedding(2,D);s.block=Block();s.outnorm=nn.LayerNorm(D);s.out=nn.Linear(D,10)
...
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  for i in range(runs):
   proposed,reg=s.transition(n,np,prob,present);active=(t>i)[:,None,None];end=torch.where(active,proposed,end);prob=torch.where(active,.5*prob+.5*proposed.softmax(-1),prob);present=torch.where(active[:,:,0],np,present)
...
def build_model(spec):
 ...
 if a!=80033:raise RuntimeError(f'state element drift: {a}')
 return m
