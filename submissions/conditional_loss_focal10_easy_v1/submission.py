"""Conditional structured-loss screen: focal10."""
...
def token_training_loss(batch:TokenLossBatch):
 ce=F.cross_entropy(batch.logits.transpose(1,2),batch.labels,ignore_index=-100,reduction='none');v=batch.valid_mask;return (ce*(1-torch.exp(-ce))*v).sum()/v.sum()
