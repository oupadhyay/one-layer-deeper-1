"""Eight-token depthwise Hyena register candidate for the Easy tier."""
...
WIDTH, D_MODEL, HIDDEN, MAX_T = 4, 128, 192, 64
STATE_ELEMENTS = 105_226
NEG = -10_000.0
...
class HyenaTick(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.norm = nn.RMSNorm(D_MODEL)
        self.qkv = nn.Linear(D_MODEL, 3 * HIDDEN, bias=False)
        self.conv = nn.Conv1d(HIDDEN, HIDDEN, 15, padding=7, groups=HIDDEN, bias=False)
        self.skip = nn.Parameter(torch.ones(HIDDEN))
        self.out = nn.Linear(HIDDEN, D_MODEL, bias=False)
        nn.init.xavier_uniform_(self.qkv.weight)
        nn.init.normal_(self.conv.weight, std=.02 / math.sqrt(15))
        nn.init.xavier_uniform_(self.out.weight)

    def forward(self, register: Tensor) -> Tensor:
        q, k, v = self.qkv(self.norm(register)).chunk(3, -1)
        r = k * v
        mixed = self.conv(r.transpose(1, 2)).transpose(1, 2)
        return register + self.out(q * (mixed + self.skip * r))


class HyenaRegister(nn.Module):
    def __init__(self) -> None:
        super().__init__()
    ...
    def make_register(self, n: Tensor, n_present: Tensor, x: Tensor,
        ...
        return torch.stack((nt, xt), 2).reshape(n.shape[0], 2 * WIDTH, D_MODEL)

    def forward(self, n: Tensor, n_present: Tensor, x: Tensor,
                x_present: Tensor) -> tuple[Tensor, Tensor]:
        register = self.make_register(n, n_present, x, x_present)
        register = self.tick(register)
        logits = self.readout(self.readout_norm(register[:, 1::2]))
        return logits, register
...
class Model(nn.Module):
    ...
    def parse(ids: Tensor, mask: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        steps = (values * powers * role.eq(3)).sum(1).long()
