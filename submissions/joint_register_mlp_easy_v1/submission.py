"""Joint-register MLP candidate for the Easy tier."""
...
WIDTH, D_MODEL, HIDDEN, MAX_T = 4, 48, 512, 64
STATE_ELEMENTS, NEG = 219_272, -10_000.0
...
class JointMLP(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.norm = nn.LayerNorm(2 * WIDTH * D_MODEL)
        self.expand = nn.Linear(2 * WIDTH * D_MODEL, HIDDEN, bias=True)
        self.project = nn.Linear(HIDDEN, WIDTH * 10, bias=True)

    def forward(self, register: Tensor) -> Tensor:
        flat = register.reshape(register.shape[0], 2 * WIDTH * D_MODEL)
        return self.project(F.gelu(self.expand(self.norm(flat)))).reshape(-1, WIDTH, 10)


class JointRegister(nn.Module):
    def __init__(self) -> None:
        super().__init__()
    ...
    def forward(self, n: Tensor, n_present: Tensor, x: Tensor,
        ...
        return self.block(register), register
