"""BiScan with one order-aware pooled-N FiLM branch before readout."""
...
class Model(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        weight = width_mask[:, :, None].to(n_states.dtype)
        summary = (n_states * weight).sum(1) / weight.sum(1).clamp_min(1.)
        scale, shift = self.film(self.value(summary)).chunk(2, -1)
        ...
        for iteration in range(executed):
            ...
            modulated = self.norm(hidden) * (1 + torch.tanh(scale[:, None])) + shift[:, None]
            modulated = torch.where(width_mask[:, :, None], modulated, torch.zeros_like(modulated))
            digit_logits = self.readout(modulated)
        ...
        if self.training:
            scale_gradient = torch.ones_like(steps, dtype=logits.dtype)
            logits = logits.detach() + scale_gradient[:, None, None] * (logits - logits.detach())
...
def build_model(spec):
    ...
    if sum(parameter.numel() for parameter in model.parameters()) != 75658:
        raise RuntimeError("conditional bidirectional scan parameter count drift")
    return model
