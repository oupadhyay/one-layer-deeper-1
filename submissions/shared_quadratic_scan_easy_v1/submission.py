"""Shared conditional quadratic recurrence over bidirectional digit states."""
...
class Transition(nn.Module):
 def __init__(s):
  super().__init__();s.digit=nn.Embedding(10,D);s.presence=nn.Embedding(2,D);s.scan=nn.GRU(D,H,batch_first=True,bidirectional=True);s.context=nn.Linear(D,3*D);s.quadratic=nn.Linear(D,D);s.linear=nn.Linear(D,D);s.norm=RMS(D);s.out=nn.Linear(D,10)
...
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  n,np=field(1);x,xp=field(2);nstate=s.transition.encode(n,np);prob=F.one_hot(x,10).to(s.transition.digit.weight.dtype);end=prob.clamp_min(1e-8).log();present=xp;runs=3 if s.training else int(t.max().item());state=None
