"""Ordered pair mixer: efficient global interactions for Easy depth learning."""
...
class Transition(nn.Module):
 def __init__(s):
  super().__init__();s.ndigit=nn.Embedding(10,D);s.xdigit=nn.Embedding(10,D);s.nplace=nn.Embedding(W,D);s.xplace=nn.Embedding(W,D);s.pair=nn.Sequential(nn.Linear(2*D,R),nn.GELU(),nn.Linear(R,R));s.mix=nn.Sequential(nn.Linear(W*W*R,H),nn.GELU(),RMS(H),nn.Linear(H,W*10))
 def forward(s,n,np,x,xp):
  ne=s.ndigit(n)+s.nplace.weight[None];xe=x.to(s.xdigit.weight.dtype)@s.xdigit.weight+s.xplace.weight[None];pairs=s.pair(torch.cat((ne[:,:,None].expand(-1,-1,W,-1),xe[:,None].expand(-1,W,-1,-1)),-1));pairs=pairs*(np[:,:,None,None]&xp[:,None,:,None]).to(pairs.dtype);return s.mix(pairs.flatten(1)).view(-1,W,10)
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  n,np=field(1);x,xp=field(2);prob=F.one_hot(x,10).to(s.transition.xdigit.weight.dtype);end=prob.clamp_min(1e-8).log();present=xp;runs=3 if s.training else int(t.max().item())
  for i in range(runs):
   proposed=s.transition(n,np,prob,present);active=(t>i)[:,None,None];end=torch.where(active,proposed,end);prob=torch.where(active,proposed.softmax(-1),prob);present=torch.where(active[:,:,0],np,present)
