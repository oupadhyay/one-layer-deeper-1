"""Tiny global quadratic state with a shared dynamic-width decoder."""
...
class Transition(nn.Module):
 def __init__(s):
  super().__init__();s.digit=nn.Embedding(10,E);s.presence=nn.Embedding(2,E);s.scan=nn.GRU(E,H,batch_first=True,bidirectional=True);s.initial=nn.Linear(E,K);s.context=nn.Linear(E,3*K);s.quadratic=nn.Linear(K,K);s.linear=nn.Linear(K,K);s.norm=RMS(K);s.local=nn.Linear(E,K);s.global_state=nn.Linear(K,K);s.readout_norm=RMS(K);s.out=nn.Linear(K,10)
 def encode(s,digit,present):
  states=s.scan(s.digit(digit)+s.presence(present.long()))[0];weight=present[...,None].to(states.dtype);return states,(states*weight).sum(1)/weight.sum(1).clamp_min(1.)
 def coefficients(s,nsummary):return s.context(nsummary).chunk(3,-1)
 def step(s,state,coefficients):
  a,b,c=coefficients;q=s.quadratic(state);return s.norm(torch.tanh(a)*q.square()+torch.tanh(b)*s.linear(state)+c)
 def decode(s,xstates,state):return s.out(s.readout_norm(s.local(xstates)+s.global_state(state)[:,None]))
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  n,np=field(1);x,xp=field(2);_,nsummary=s.transition.encode(n,np);xstates,xsummary=s.transition.encode(x,xp);state=torch.tanh(s.transition.initial(xsummary));coefficients=s.transition.coefficients(nsummary);end=state.new_zeros(B,width,10);runs=int(t.max().item())
  for step in range(runs):
   proposed=s.transition.step(state,coefficients);active=(t>step)[:,None];state=torch.where(active,proposed,state);end=torch.where(active[:,:,None],s.transition.decode(xstates,state),end)
...
def build_optimizer(m,spec):
 ...
 o=torch.optim.AdamW([{'params':d,'weight_decay':.01},{'params':n,'weight_decay':0.}],lr=2e-3,betas=(.9,.95),eps=1e-8,capturable=spec.device_type=='cuda');return OptimizerBundle(o,torch.optim.lr_scheduler.LambdaLR(o,lambda u:min((u+1)/8,1.)))
