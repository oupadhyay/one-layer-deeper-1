"""Width-shared three-tap place-mixing SGU for Easy E5."""
...
class Block(nn.Module):
 def __init__(s):
  super().__init__();s.norm=RMS(D);s.expand=nn.Linear(D,2*H);s.gnorm=nn.LayerNorm(H);s.kernel=nn.Parameter(torch.zeros(3));s.role=nn.Parameter(torch.empty(W,2,2));s.bias=nn.Parameter(torch.ones(W,2));s.project=nn.Linear(H,D);nn.init.uniform_(s.role,-1.25e-4,1.25e-4)
 def forward(s,x):
  u,v=F.gelu(s.expand(s.norm(x))).chunk(2,-1);v=s.gnorm(v).reshape(x.shape[0],W,2,H);k=torch.tanh(s.kernel);k=k/k.abs().sum().clamp_min(1.);z=F.pad(v,(0,0,0,0,1,1));p=k[0]*z[:,:W]+k[1]*z[:,1:W+1]+k[2]*z[:,2:W+2];g=(p+torch.einsum('por,bprh->bpoh',s.role,v)+s.bias[None,:,:,None]).reshape_as(u);return x+s.project(u*g)
...
def build_model(spec):
 ...
 if a!=79669: raise RuntimeError(f'state element drift: {a}')
 return m
