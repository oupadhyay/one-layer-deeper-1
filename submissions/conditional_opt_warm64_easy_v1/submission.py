"""Conditional optimizer screen: warm64."""
...
def build_optimizer(m,spec):
 ...
 opt=torch.optim.AdamW([{'params':decay,'weight_decay':.01},{'params':no_decay,'weight_decay':0.}],lr=6e-4,betas=(.9,.95),eps=1e-8,capturable=spec.device_type=='cuda');return OptimizerBundle(opt,torch.optim.lr_scheduler.LambdaLR(opt,lambda u:min((u+1)/64,1.)))
