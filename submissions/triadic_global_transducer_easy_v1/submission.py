"""Global triadic digit-relation transducer for Easy T1 learning."""
...
class Transition(nn.Module):
 def __init__(s):
  super().__init__();s.ndigit=nn.Embedding(10,D);s.xdigit=nn.Embedding(10,D);s.presence=nn.Embedding(2,D);s.nplace=nn.Embedding(W,D);s.xplace=nn.Embedding(W,D);s.oplace=nn.Embedding(W,D);s.relation=nn.Sequential(nn.Linear(5*D,H),nn.GELU(),nn.Linear(H,H));s.norm=RMS(H);s.out=nn.Linear(H,10)
 def forward(s,n,np,x,xp):
  B=n.shape[0];ne=s.ndigit(n)+s.nplace.weight[None]+s.presence(np.long());xe=x.to(s.xdigit.weight.dtype)@s.xdigit.weight+s.xplace.weight[None]+s.presence(xp.long());ni=ne[:,None,None].expand(B,W,W,W,D);xj=xe[:,None,:,None].expand(B,W,W,W,D);op=s.oplace.weight[None,:,None,None].expand(B,W,W,W,D);dx=(s.oplace.weight[:,None]-s.xplace.weight[None])[None,:,:,None].expand(B,W,W,W,D);dn=(s.oplace.weight[:,None]-s.nplace.weight[None])[None,:,None,:].expand(B,W,W,W,D);r=s.relation(torch.cat((ni,xj,op,dx,dn),-1));mask=xp[:,None,:,None]&np[:,None,None,:];weight=mask[...,None].to(r.dtype);h=(r*weight).sum((2,3))/weight.sum((2,3)).clamp_min(1.);return s.out(s.norm(h)),h
class Model(nn.Module):
 ...
 def forward(s,ids,attention_mask=None):
  ...
  n,np=field(1);x,xp=field(2);prob=F.one_hot(x,10).to(s.transition.xdigit.weight.dtype);end=prob.clamp_min(1e-8).log();present=xp;runs=3 if s.training else int(t.max().item());state=None
  for i in range(runs):
   proposed,state=s.transition(n,np,prob,present);active=(t>i)[:,None,None];end=torch.where(active,proposed,end);prob=torch.where(active,proposed.softmax(-1),prob);present=torch.where(active[:,:,0],np,present)
