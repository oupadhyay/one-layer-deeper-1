"""C21 SGU with under-relaxed recurrent categorical feedback."""
...
STATE_ELEMENTS,NEG=79_682,-10_000.0
class Config:
...
class SGURegister(nn.Module):
 ...
 def forward(self,n,np,x,xp):
  dt=self.digit_embedding.weight.dtype; a=self.digit_embedding(n)*np[...,None].to(dt); b=(x.to(dt)@self.digit_embedding.weight)*xp[...,None].to(dt); p=self.place_embedding.weight[None]; r=torch.stack((a+self.role_embedding.weight[0]+p+self.presence_embedding(np.long()),b+self.role_embedding.weight[1]+p+self.presence_embedding(xp.long())),2).reshape(n.shape[0],2*WIDTH,D_MODEL); r=self.block(r); return self.readout(self.readout_norm(r[:,1::2])),r
class Model(nn.Module):
 ...
 def forward(self,ids,attention_mask=None):
  ...
  if bool(((nw<1)|(nw>WIDTH)|(xw<1)|(xw>WIDTH)).any()): raise ValueError("N and X widths must be between one and four")
  if bool((steps>64).any()): raise ValueError("T exceeds 64")
  slots=torch.arange(WIDTH,device=ids.device); val=(ids-7).clamp(0,9)
  def field(k):
   q=role.eq(k)[:,:,None]&place[:,:,None].eq(slots); return (val[:,:,None]*q).sum(1).long(),q.any(1)
  n,np=field(1); x,xp=field(2); prob=F.one_hot(x,10).to(self.transition.digit_embedding.weight.dtype); end=prob.clamp_min(1e-8).log(); present=xp; runs=3 if self.training else int(steps.max().item()); reg=None
  for i in range(runs):
   proposed,reg=self.transition(n,np,prob,present); active=(steps>i)[:,None,None]; end=torch.where(active,proposed,end); next_prob=.5*prob+.5*proposed.softmax(-1); prob=torch.where(active,next_prob,prob); present=torch.where(active[:,:,0],np,present)
