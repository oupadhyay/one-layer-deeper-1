"""Standalone calibrated C12 with diagonal bypass for the Easy E5 tier."""
...
STATE_ELEMENTS, NEG = 79_459, -10_000.0
...
class RMSNorm(nn.Module):
    ...
    def forward(self, value: Tensor) -> Tensor:
        return F.rms_norm(value, (self.width,), self.weight, self.eps)


class SpatialGatingBlock(nn.Module):
    """gMLP SGU with a generic Kronecker-sum place/role operator."""
    def __init__(self) -> None:
        super().__init__()
        self.norm = RMSNorm(D_MODEL)
        self.expand = nn.Linear(D_MODEL, 2 * HIDDEN, bias=True)
        self.gate_norm = RMSNorm(HIDDEN)
        self.place_operator = nn.Linear(WIDTH, WIDTH, bias=False)
        self.role_operator = nn.Linear(2, 2, bias=False)
        self.alpha = nn.Parameter(torch.zeros(()))
        self.diagonal = nn.Parameter(torch.zeros(WIDTH, 2))
        self.place_bias = nn.Parameter(torch.full((WIDTH,), 0.5))
        self.role_bias = nn.Parameter(torch.full((2,), 0.5))
        self.place_gain = nn.Parameter(torch.ones(WIDTH))
        self.role_gain = nn.Parameter(torch.ones(2))
        self.project = nn.Linear(HIDDEN, D_MODEL, bias=True)
        nn.init.uniform_(self.place_operator.weight, -1.25e-4, 1.25e-4)
        nn.init.uniform_(self.role_operator.weight, -1.25e-4, 1.25e-4)

    def forward(self, register: Tensor) -> Tensor:
        u, v = F.gelu(self.expand(self.norm(register))).chunk(2, dim=-1)
        v = self.gate_norm(v).reshape(v.shape[0], WIDTH, 2, HIDDEN)
        place_gate = self.place_operator(v.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        role_gate = self.role_operator(v.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)
        interaction = self.place_operator(role_gate.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        gate = (place_gate + role_gate + self.alpha * interaction
                + self.diagonal[None, :, :, None] * v
                + self.place_bias[None, :, None, None]
                + self.role_bias[None, None, :, None])
        gate = (gate * self.place_gain[None, :, None, None]
                * self.role_gain[None, None, :, None]).reshape_as(u)
        return register + self.project(u * gate)
...
class SGURegister(nn.Module):
    def __init__(self) -> None:
        ...
        self.readout_norm = RMSNorm(D_MODEL)
