"""Compressed axial C64 control with shared row/column decimal-place encoding."""
import torch
...
MAX_STEPS, CHANNELS, PLACE_TABLE_SIZE, BOUNDARY_PLACE, STATE_ELEMENTS = 64, 64, 65, 64, 49722
...
class Model(nn.Module):
    def __init__(self, spec):
        ...
        # One table is deliberately shared by both workspace axes.  The final
        # entry identifies the answer boundary column.
        self.place_embedding = nn.Embedding(PLACE_TABLE_SIZE, 16)
        self.initializer = nn.Sequential(nn.Linear(102, CHANNELS), nn.GELU(), nn.Linear(CHANNELS, CHANNELS))
    ...
    def _workspace(self, probabilities, n_digits, widths):
        ...
        ej = torch.cat((state[:, None, :].expand(-1, width, -1, -1), zero), 2)
        nj = torch.cat((n_state[:, None, :].expand(-1, width, -1, -1), zero), 2)
        ...
        # Place indices are canonical (least-significant place first), not
        # normalized to each example's width. Boundary always uses one
        # dedicated, width-independent table entry.
        row_place = self.place_embedding(torch.arange(width, device=state.device))
        row_place = row_place[None, :, None].expand(batch, -1, columns, -1)
        col_index = torch.arange(columns, device=state.device)[None, None, :].expand(batch, width, -1)
        col_index = torch.where(boundary.expand(-1, width, -1), BOUNDARY_PLACE, col_index)
        col_place = self.place_embedding(col_index)
        valid = valid_row & (pair | boundary)
        workspace = self.initializer(torch.cat((ei, ej, ni, nj, row_place, col_place, flags), -1))
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        ungated = logits
        if self.training:
            scale = steps.eq(1).to(logits.dtype)
...
def build_model(spec):
    ...
    if sum(p.numel() for p in model.parameters()) != STATE_ELEMENTS:
        raise RuntimeError("compressed axial place-encoding parameter count drift")
    return model

def build_optimizer(model, spec):
    return OptimizerBundle(torch.optim.AdamW(model.parameters(), lr=1e-4, betas=(0.9, 0.95),
