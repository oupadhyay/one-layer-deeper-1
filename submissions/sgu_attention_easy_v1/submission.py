"""Normalized content-attention place-mixing SGU for Easy E5."""
...
class Block(nn.Module):
 def __init__(s):
  super().__init__();s.norm=RMS(D);s.expand=nn.Linear(D,2*H);s.gnorm=nn.LayerNorm(H);s.query=nn.Linear(H,16,bias=False);s.key=nn.Linear(H,16,bias=False);s.role=nn.Parameter(torch.empty(W,2,2));s.bias=nn.Parameter(torch.ones(W,2));s.project=nn.Linear(H,D);nn.init.uniform_(s.role,-1.25e-4,1.25e-4)
 def forward(s,x):
  u,v=F.gelu(s.expand(s.norm(x))).chunk(2,-1);v=s.gnorm(v).reshape(x.shape[0],W,2,H);summary=v.mean(2);weights=(s.query(summary)@s.key(summary).transpose(1,2)/4.).softmax(-1);p=torch.einsum('bop,bprh->borh',weights,v);g=(p+torch.einsum('por,bprh->bpoh',s.role,v)+s.bias[None,:,:,None]).reshape_as(u);return x+s.project(u*g)
...
def build_model(spec):
 ...
 if a!=86834: raise RuntimeError(f'state element drift: {a}')
 return m
