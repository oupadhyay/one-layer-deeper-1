"""Standalone calibrated C33 with diagonal bypass for the Easy E5 tier."""
...
STATE_ELEMENTS, NEG = 79_570, -10_000.0
...
class SpatialGatingBlock(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.place_operator = nn.Parameter(torch.empty(WIDTH, WIDTH))
        self.role_operator = nn.Parameter(torch.empty(2, 2))
        ...
        nn.init.uniform_(self.place_operator, -1.25e-4, 1.25e-4)
        nn.init.uniform_(self.role_operator, -1.25e-4, 1.25e-4)
    ...
    def forward(self, register: Tensor) -> Tensor:
        ...
        place_gate = torch.einsum("op,bprh->borh", self.place_operator, v)
        role_gate = torch.einsum("or,bprh->bpoh", self.role_operator, v)
        gate = (place_gate + role_gate + self.diagonal[None, :, :, None] * v
