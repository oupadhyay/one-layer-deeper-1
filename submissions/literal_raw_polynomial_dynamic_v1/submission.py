"""N-conditioned literal polynomial recurrence over a persistent latent state."""
...
class Transition(nn.Module):
 ...
 def step(s,state,coefficients,present):
  a,b,c=coefficients;next_state=s.norm(torch.tanh(a[:,None])*state.square()+torch.tanh(b[:,None])*state+c[:,None]);return next_state*present[...,None].to(next_state.dtype)
