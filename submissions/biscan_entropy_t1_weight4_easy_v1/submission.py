"""BiScan entropy preference with fourfold T1 endpoint weighting."""
...
def token_training_loss(batch: TokenLossBatch):
    ...
    horizon_weight = torch.where(batch.auxiliary["parsed_steps"].eq(1),
                                 losses.new_tensor(4.), losses.new_tensor(1.))[:, None]
    weights = valid * horizon_weight
    endpoint_loss = (losses * weights).sum() / weights.sum().clamp_min(1.)
    return endpoint_loss + .01 * batch.auxiliary["feedback_entropy"]
