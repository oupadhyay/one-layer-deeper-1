...
def token_training_loss(batch) -> Tensor:
    """Boundary-preserving blend of mean and soft worst-token CE."""
    logits = batch.logits
    labels = batch.labels
    valid = batch.valid_mask.to(device=logits.device, dtype=torch.bool)
    safe_labels = torch.where(valid, labels, torch.zeros_like(labels))
    losses = F.cross_entropy(
        logits.reshape(-1, logits.shape[-1]), safe_labels.reshape(-1), reduction="none"
    ).reshape_as(labels)
    counts = valid.sum(dim=1)
    nonempty = counts > 0
    counts_float = counts.clamp_min(1).to(dtype=losses.dtype)
    mean_ce = (losses * valid).sum(dim=1) / counts_float
    tau = 0.5
    masked_losses = losses.masked_fill(~valid, float("-inf"))
    soft_worst = tau * (
        torch.logsumexp(masked_losses / tau, dim=1) - counts_float.log()
    )
    per_example = 0.5 * mean_ce + 0.5 * soft_worst
    if nonempty.any():
        return per_example[nonempty].mean()
    return logits.sum() * 0.0


def legacy_training_loss(logits: Tensor, labels: Tensor, auxiliary: object) -> Tensor:
    """Compatibility for checkouts predating token-level loss batches."""
...
if "token_training_loss" in Submission.__dataclass_fields__:
    submission_kwargs["token_training_loss"] = token_training_loss
else:
    submission_kwargs["training_loss"] = legacy_training_loss
