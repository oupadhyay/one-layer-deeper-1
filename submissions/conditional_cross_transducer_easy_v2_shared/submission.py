"""Low-capacity dual-stream transducer with one shared digit scanner."""
...
class Transition(nn.Module):
 def __init__(s):
  super().__init__();s.digit=nn.Embedding(10,D);s.presence=nn.Embedding(2,D);s.scan=nn.GRU(D,H,batch_first=True,bidirectional=True);s.q=nn.Linear(D,A,bias=False);s.k=nn.Linear(D,A,bias=False);s.v=nn.Linear(D,D,bias=False);s.content=nn.Linear(2*D,D);s.film=nn.Linear(D,2*D);s.norm=RMS(D);s.out=nn.Linear(D,10)
 def encode(s,digit,present):
  state=s.digit(digit)+s.presence(present.long());return s.scan(state)[0]
 def forward(s,nstate,np,x,xp):
  state=x.to(s.digit.weight.dtype)@s.digit.weight;state=state+s.presence(xp.long());xs=s.scan(state)[0];score=(s.q(xs)@s.k(nstate).transpose(1,2))/(A**.5);score=score.masked_fill(~np[:,None,:],-10000.);ctx=score.softmax(-1)@s.v(nstate);h=F.gelu(s.content(torch.cat((xs,ctx),-1)));scale,shift=s.film(ctx).chunk(2,-1);h=s.norm(h)*(1+torch.tanh(scale))+shift;return s.out(h),h
...
def build_model(spec):
 ...
 if actual!=43018:raise RuntimeError(f'state element drift: {actual}')
 return m
def build_optimizer(m,spec):
 ...
 opt=torch.optim.AdamW([{'params':decay,'weight_decay':.01},{'params':no_decay,'weight_decay':0.}],lr=6e-4,betas=(.9,.95),eps=1e-8,capturable=spec.device_type=='cuda');return OptimizerBundle(opt,torch.optim.lr_scheduler.LambdaLR(opt,lambda u:min((u+1)/32,1.)))
