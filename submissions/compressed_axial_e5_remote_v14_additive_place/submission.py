"""Compute-efficient compressed axial model with additive place encoding."""
...
MAX_STEPS, CHANNELS, PLACE_TABLE_SIZE, BOUNDARY_PLACE, STATE_ELEMENTS = 64, 64, 65, 64, 47674
...
class Model(nn.Module):
    def __init__(self, spec):
        ...
        # Decimal places share one table across both axes; index 64 is the
        # width-independent answer-boundary column.
    ...
    def _workspace(self, probabilities, n_digits, widths):
        ...
        # Boundary digit state was zeroed above before its dedicated place is added.
        ej = ej + self.place_embedding(col_index)
...
def build_model(spec):
    ...
    if sum(parameter.numel() for parameter in model.parameters()) != STATE_ELEMENTS:
        raise RuntimeError("compressed axial additive-place parameter count drift")
    return model
