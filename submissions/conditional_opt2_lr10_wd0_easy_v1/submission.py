"""Conditional optimizer phase 2: lr10_wd0."""
...
def build_optimizer(m,spec):
 ...
 opt=torch.optim.AdamW([{'params':decay,'weight_decay':0.},{'params':no_decay,'weight_decay':0.}],lr=1e-3,betas=(.9,.95),eps=1e-8,capturable=spec.device_type=='cuda');return OptimizerBundle(opt,torch.optim.lr_scheduler.LambdaLR(opt,lambda u:min((u+1)/32,1.)))
